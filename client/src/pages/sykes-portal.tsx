...
import {
  Truck, MapPin, Plus, FileText, Printer, LogOut, DollarSign,
  ClipboardList, CheckCircle2, Clock, X, Loader2, Trash2
} from "lucide-react";
...
// ─── Branding constants ───────────────────────────────────────────
const BRAND = { navy: "#1e3a5f", orange: "#f97316", light: "#f8fafc", muted: "#64748b" };
...
// ─── Sykes Invoice Print ──────────────────────────────────────────
function printSykesInvoice(invoice: InvoiceWithExtras) {
  ...
  <body><div style="max-width:760px;margin:0 auto;padding:40px">
    <div style="height:5px;background:linear-gradient(90deg,#1e3a5f,#f97316);border-radius:3px;margin-bottom:24px"></div>
    <div style="display:flex;justify-content:space-between;align-items:flex-start;margin-bottom:24px">
      <div>
        <div style="display:flex;align-items:center;gap:10px;margin-bottom:4px">
          <div style="width:48px;height:48px;border-radius:50%;background:#1e3a5f;display:flex;align-items:center;justify-content:center">
            <span style="color:#f97316;font-weight:900;font-size:18px">S&amp;S</span>
          </div>
          <div>
            <div style="font-size:20px;font-weight:800;color:#1e3a5f">Sykes &amp; Sons Logistics</div>
            <div style="font-size:12px;color:#64748b">Texas City, TX 77372 &bull; (832) 555-0100</div>
          </div>
        </div>
      </div>
      <div style="text-align:right">
        <div style="font-size:30px;font-weight:800;color:#f97316;letter-spacing:2px">INVOICE</div>
    ...
    <div style="display:flex;justify-content:space-between;margin-bottom:28px">
      <div>
        <div style="font-size:10px;color:#f97316;font-weight:700;letter-spacing:1.5px;margin-bottom:6px">BILL TO</div>
      ...
      <div style="text-align:right">
        <div style="font-size:10px;color:#f97316;font-weight:700;letter-spacing:1.5px;margin-bottom:6px">INVOICE DETAILS</div>
    ...
    <div style="display:flex;justify-content:flex-end;margin-bottom:28px">
      <div style="width:260px">
        ...
        <div style="display:flex;justify-content:space-between;align-items:center;padding:10px 14px;margin-top:8px;background:#f97316;border-radius:6px;color:#fff">
          <span style="font-size:14px;font-weight:700">TOTAL DUE:</span>
          <span style="font-size:20px;font-weight:800">$${total.toFixed(2)}</span>
        </div>
      </div>
    </div>
    ${invoice.notes ? `<div style="margin-bottom:20px"><div style="font-size:10px;color:#f97316;font-weight:700;letter-spacing:1.5px;margin-bottom:6px">NOTES</div><div style="font-size:12px;color:#1e293b;line-height:1.6">${invoice.notes}</div></div>` : ""}
    <div style="border-top:2px solid #f97316;padding-top:14px;text-align:center">
      <div style="font-size:12px;font-weight:800;color:#1e3a5f">SYKES &amp; SONS LOGISTICS LLC</div>
      <div style="font-size:10px;color:#94a3b8;margin-top:4px">Texas City, TX 77372 &bull; info@sykesandsonlogistics.com &bull; Moving Texas Forward</div>
...
export default function SykesPortal() {
  ...
  const [formData, setFormData] = useState({ client_id: "", notes: "", status: "draft", tax_rate: "0" });
  ...
  useEffect(() => {
    if (!authLoading && !user) navigate("/sykes");
  ...
  const resetForm = () => {
    ...
    setFormData({ client_id: "", notes: "", status: "draft", tax_rate: "0" });
  };
  ...
  if (authLoading) {
    return (
      <div style={{ minHeight: "100vh", background: BRAND.navy, display: "flex", alignItems: "center", justifyContent: "center" }}>
        <Loader2 size={48} color={BRAND.orange} className="animate-spin" />
      </div>
    );
  }
  ...
  return (
    <div style={{ minHeight: "100vh", background: "#f1f5f9", fontFamily: "system-ui, sans-serif" }}>
      {/* ── Header ── */}
      <header style={{ background: BRAND.navy, padding: "0 24px", height: 64, display: "flex", alignItems: "center", justifyContent: "space-between" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
          <div style={{ width: 40, height: 40, borderRadius: "50%", background: "rgba(249,115,22,0.15)", border: "2px solid #f97316", display: "flex", alignItems: "center", justifyContent: "center" }}>
            <Truck size={20} color="#f97316" />
          </div>
          <div>
            <div style={{ color: "#fff", fontWeight: 800, fontSize: 18, lineHeight: 1 }}>Sykes &amp; Sons Logistics</div>
            <div style={{ color: "#94a3b8", fontSize: 11, display: "flex", alignItems: "center", gap: 4 }}>
              <MapPin size={10} /><span>Texas City, TX 77372</span>
            </div>
          </div>
        </div>
        <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
          <span style={{ color: "#f97316", fontSize: 12, fontWeight: 600, background: "rgba(249,115,22,0.1)", padding: "4px 10px", borderRadius: 6, border: "1px solid rgba(249,115,22,0.3)" }}>
            Vendor Portal
          </span>
          <button onClick={handleLogout} style={{ background: "none", border: "none", cursor: "pointer", color: "#94a3b8", display: "flex", alignItems: "center", gap: 6, fontSize: 13 }}>
      ...
      <main style={{ maxWidth: 1100, margin: "0 auto", padding: "32px 24px" }}>
        ...
        <div style={{ marginBottom: 28 }}>
          <h1 style={{ fontSize: 26, fontWeight: 800, color: BRAND.navy }}>Invoice Dashboard</h1>
          <p style={{ color: BRAND.muted, fontSize: 14, marginTop: 4 }}>Create, manage, and print invoices for your logistics services.</p>
        </div>

        {/* Stats */}
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(200px,1fr))", gap: 16, marginBottom: 28 }}>
          {[
            { icon: <ClipboardList size={22} color={BRAND.orange} />, label: "Total Invoices", value: invoices.length },
            ...
            { icon: <DollarSign size={22} color={BRAND.orange} />, label: "Revenue Collected", value: `$${totalRev.toLocaleString("en-US", { minimumFractionDigits: 2 })}` },
          ].map((s, i) => (
            <div key={i} style={{ background: "#fff", borderRadius: 12, padding: "20px 24px", border: "1px solid #e2e8f0", display: "flex", alignItems: "center", gap: 14 }}>
              <div style={{ width: 44, height: 44, borderRadius: 10, background: "#f8fafc", display: "flex", alignItems: "center", justifyContent: "center" }}>{s.icon}</div>
              <div>
                <div style={{ fontSize: 22, fontWeight: 800, color: BRAND.navy }}>{s.value}</div>
        ...
        <div style={{ background: "#fff", borderRadius: 14, border: "1px solid #e2e8f0", overflow: "hidden" }}>
          <div style={{ padding: "20px 24px", borderBottom: "1px solid #e2e8f0", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
            <div>
              <h2 style={{ fontSize: 18, fontWeight: 700, color: BRAND.navy }}>Invoices</h2>
              <p style={{ fontSize: 13, color: BRAND.muted, marginTop: 2 }}>{invoices.length} invoice{invoices.length !== 1 ? "s" : ""} on file</p>
            </div>
            <Button onClick={() => setShowCreate(true)} style={{ background: BRAND.orange, color: "#fff", border: "none", fontWeight: 700 }}>
                ...
                <tbody>
                  {invoices.map((inv, i) => (
                    <tr key={inv.id} style={{ borderTop: "1px solid #f1f5f9", background: i % 2 === 0 ? "#fff" : "#fafafa" }}>
                      <td style={{ padding: "14px 16px", fontWeight: 700, color: BRAND.navy, fontSize: 14 }}>{inv.invoice_number}</td>
                      ...
                      <td style={{ padding: "14px 16px", fontWeight: 700, color: BRAND.navy }}>${parseFloat(inv.total).toFixed(2)}</td>
                      ...
                      <td style={{ padding: "14px 16px" }}>
                        <div style={{ display: "flex", gap: 8 }}>
                          <button
                            ...
                            style={{ background: BRAND.navy, color: "#fff", border: "none", borderRadius: 6, padding: "6px 12px", fontSize: 12, fontWeight: 600, cursor: "pointer", display: "flex", alignItems: "center", gap: 4 }}>
                            <FileText size={13} />View
                          </button>
                          <button
                            onClick={() => printSykesInvoice(inv)}
                            style={{ background: BRAND.orange, color: "#fff", border: "none", borderRadius: 6, padding: "6px 12px", fontSize: 12, fontWeight: 600, cursor: "pointer", display: "flex", alignItems: "center", gap: 4 }}>
      ...
      {showCreate && (
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.6)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 1000, padding: 16 }}>
          <div style={{ background: "#fff", borderRadius: 16, width: "100%", maxWidth: 700, maxHeight: "90vh", overflowY: "auto", boxShadow: "0 25px 60px rgba(0,0,0,0.3)" }}>
            {/* Modal header */}
            <div style={{ padding: "20px 24px", borderBottom: "1px solid #e2e8f0", display: "flex", alignItems: "center", justifyContent: "space-between", background: BRAND.navy, borderRadius: "16px 16px 0 0" }}>
              <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                <Plus size={20} color={BRAND.orange} />
                <h3 style={{ color: "#fff", fontWeight: 700, fontSize: 18 }}>Create New Invoice</h3>
              </div>
              <button onClick={() => { setShowCreate(false); resetForm(); }} style={{ background: "none", border: "none", cursor: "pointer", color: "#94a3b8" }}>
                <X size={22} />
              </button>
            </div>
            ...
            <form onSubmit={handleCreateSubmit} style={{ padding: 24 }}>
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16, marginBottom: 20 }}>
                <div>
                  <Label style={{ color: BRAND.navy, fontWeight: 600 }}>Client *</Label>
                ...
                <div>
                  <Label style={{ color: BRAND.navy, fontWeight: 600 }}>Status</Label>
              ...
              <div style={{ marginBottom: 20 }}>
                <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 10 }}>
                  <Label style={{ color: BRAND.navy, fontWeight: 600 }}>Line Items</Label>
                  <button type="button" onClick={() => setLineItems(p => [...p, { description: "", quantity: "1", unit_price: "" }])}
                    style={{ background: BRAND.navy, color: "#fff", border: "none", borderRadius: 6, padding: "5px 12px", fontSize: 12, fontWeight: 600, cursor: "pointer", display: "flex", alignItems: "center", gap: 4 }}>
                    <Plus size={13} />Add Line
                  </button>
                </div>
                <div style={{ border: "1px solid #e2e8f0", borderRadius: 10, overflow: "hidden" }}>
                  <div style={{ display: "grid", gridTemplateColumns: "1fr 80px 120px 40px", gap: 0, background: "#f8fafc", padding: "10px 14px" }}>
                  ...
                  {lineItems.map((it, i) => (
                    <div key={i} style={{ display: "grid", gridTemplateColumns: "1fr 80px 120px 40px", gap: 8, padding: "10px 14px", borderTop: "1px solid #f1f5f9", alignItems: "center" }}>
              ...
              {(() => { const { subtotal, taxAmount, total } = calcTotals(); return (
                <div style={{ display: "flex", justifyContent: "flex-end", marginBottom: 20 }}>
                  <div style={{ width: 240, padding: "12px 16px", background: "#f8fafc", borderRadius: 10, border: "1px solid #e2e8f0" }}>
                    ...
                    <div style={{ display: "flex", justifyContent: "space-between", padding: "8px 12px", background: BRAND.orange, borderRadius: 8, color: "#fff" }}>
              ...
              <div style={{ marginBottom: 20 }}>
                <Label style={{ color: BRAND.navy, fontWeight: 600 }}>Notes (optional)</Label>
                <Textarea value={formData.notes} onChange={e => setFormData(p => ({ ...p, notes: e.target.value }))} placeholder="Any special instructions or notes for this invoice…" style={{ marginTop: 6, resize: "vertical" }} rows={3} />
              </div>

              <div style={{ display: "flex", gap: 12, justifyContent: "flex-end" }}>
                <Button type="button" variant="outline" onClick={() => { setShowCreate(false); resetForm(); }}>Cancel</Button>
                <Button type="submit" disabled={createMutation.isPending} style={{ background: BRAND.orange, color: "#fff", border: "none", fontWeight: 700 }}>
      ...
      {viewInvoice && (
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.6)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 1000, padding: 16 }}>
          <div style={{ background: "#fff", borderRadius: 16, width: "100%", maxWidth: 680, maxHeight: "90vh", overflowY: "auto", boxShadow: "0 25px 60px rgba(0,0,0,0.3)" }}>
            <div style={{ padding: "16px 24px", borderBottom: "1px solid #e2e8f0", display: "flex", alignItems: "center", justifyContent: "space-between", background: BRAND.navy, borderRadius: "16px 16px 0 0" }}>
              <h3 style={{ color: "#fff", fontWeight: 700 }}>Invoice {viewInvoice.invoice_number}</h3>
              <div style={{ display: "flex", gap: 8 }}>
                <button onClick={() => printSykesInvoice(viewInvoice)}
                  style={{ background: BRAND.orange, color: "#fff", border: "none", borderRadius: 6, padding: "7px 14px", fontSize: 13, fontWeight: 600, cursor: "pointer", display: "flex", alignItems: "center", gap: 6 }}>
                  <Printer size={15} />Print / Save PDF
                </button>
                <button onClick={() => setViewInvoice(null)} style={{ background: "none", border: "none", cursor: "pointer", color: "#94a3b8" }}>
                  <X size={22} />
                </button>
              </div>
            </div>
            <div style={{ padding: 28, color: "#1e293b" }}>
              ...
              <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 24 }}>
                <div>
                  <div style={{ fontWeight: 800, fontSize: 17, color: BRAND.navy }}>Sykes &amp; Sons Logistics</div>
                  <div style={{ color: BRAND.muted, fontSize: 12, marginTop: 2 }}>Texas City, TX 77372</div>
                </div>
                <div style={{ textAlign: "right" }}>
                  <div style={{ fontSize: 13, color: BRAND.muted }}>Invoice #: <strong style={{ color: BRAND.navy }}>{viewInvoice.invoice_number}</strong></div>
                  <div style={{ fontSize: 13, color: BRAND.muted, marginTop: 4 }}>Date: <strong style={{ color: BRAND.navy }}>{new Date(viewInvoice.invoice_date ?? viewInvoice.created_at).toLocaleDateString("en-US")}</strong></div>
              ...
              <div style={{ marginBottom: 16 }}>
                <div style={{ fontSize: 10, color: BRAND.orange, fontWeight: 700, letterSpacing: 1.5, marginBottom: 6 }}>BILL TO</div>
                ...
                <table style={{ width: "100%", borderCollapse: "collapse", marginBottom: 16 }}>
                  ...
                  <tbody>
                    {(viewInvoice.items ?? []).map((it, i) => (
                      <tr key={it.id} style={{ background: i % 2 === 0 ? "#f8fafc" : "#fff" }}>
                        <td style={{ padding: "8px 12px", fontSize: 13, borderBottom: "1px solid #f1f5f9" }}>{it.description}</td>
                        <td style={{ padding: "8px 12px", fontSize: 13, textAlign: "right", borderBottom: "1px solid #f1f5f9" }}>{parseFloat(it.quantity)}</td>
                        <td style={{ padding: "8px 12px", fontSize: 13, textAlign: "right", borderBottom: "1px solid #f1f5f9" }}>${parseFloat(it.unit_price).toFixed(2)}</td>
                        <td style={{ padding: "8px 12px", fontSize: 13, textAlign: "right", borderBottom: "1px solid #f1f5f9", fontWeight: 700 }}>${parseFloat(it.amount).toFixed(2)}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
              ...
              <div style={{ display: "flex", justifyContent: "flex-end" }}>
                <div style={{ width: 240 }}>
                  ...
                  <div style={{ display: "flex", justifyContent: "space-between", padding: "10px 14px", background: BRAND.orange, borderRadius: 8, color: "#fff", marginTop: 8 }}>
              ...
              {viewInvoice.notes && (
                <div style={{ marginTop: 16, padding: "10px 14px", background: "#f8fafc", borderRadius: 8, fontSize: 13, color: BRAND.muted }}>
                  <strong style={{ color: BRAND.navy }}>Notes:</strong> {viewInvoice.notes}
                </div>
              )}
            </div>
          </div>
